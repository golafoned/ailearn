export class ApiError extends Error {
    ...
    static generationIncomplete(
        message = "Generation incomplete",
        code = "GENERATION_INCOMPLETE"
    ) {
        return new ApiError(500, code, message);
    }
    static reviewInvalidContext(
        message = "Invalid review context",
        code = "REVIEW_INVALID_CONTEXT"
    ) {
        return new ApiError(400, code, message);
    }
    static reviewStrategyUnsupported(
        message = "Unsupported review strategy",
        code = "REVIEW_STRATEGY_UNSUPPORTED"
    ) {
        return new ApiError(400, code, message);
    }
    static reviewGenerationFailed(
