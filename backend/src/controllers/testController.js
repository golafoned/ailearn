...
export async function getTestByCode(req, res, next) {
  try {
    ...
    const questions = JSON.parse(test.questions_json).map(q => ({ id: q.id, type: q.type, question: q.question, options: q.options || [] }));
    res.json({ code: test.code, title: test.title, expiresAt: test.expires_at, timeLimitSeconds: test.time_limit_seconds, questions });
  } catch (e) { next(e); }
}
...
export async function submitAttempt(req, res, next) {
  try {
    ...
    const answerRecords = answers.map(a => {
      ...
      const isCorrect = q.answer != null && a.answer != null ? String(q.answer).trim().toLowerCase() === String(a.answer).trim().toLowerCase() : null;
      return {
    ...
    const numericScoreParts = answerRecords.filter(r=>r.correct_answer!=null && r.user_answer!=null);
    const correctCount = numericScoreParts.filter(r=>r.is_correct===1).length;
    const score = numericScoreParts.length ? Math.round((correctCount / numericScoreParts.length)*100) : null;
    ...
    res.json({ attemptId: updated.id, submittedAt: updated.submitted_at, score: updated.score, totalQuestions: questions.length, answered: answers.length });
  } catch (e) { next(e); }
}
...
export async function listTestAttempts(req, res, next) {
  try {
    ...
    if (!test) return res.status(404).json({ error: 'Test not found' });
    if (test.created_by && test.created_by !== req.user?.id) return res.status(403).json({ error: 'Forbidden' });
    const attempts = await attemptRepo.listByTest(testId);
    res.json({ attempts: attempts.map(a=>({ id: a.id, userId: a.user_id, participantName: a.participant_name, displayName: a.display_name, startedAt: a.started_at, submittedAt: a.submitted_at, score: a.score })) });
  } catch (e) { next(e); }
}
