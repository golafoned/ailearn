...
export class PracticeSessionRepository {
    async create(session) {
        const conceptsJson = session.concepts_json
            ...
            : null;
        await run(`INSERT INTO practice_sessions (
    ...
    async findById(id) {
        return queryOne(`SELECT * FROM practice_sessions WHERE id=? LIMIT 1`, [id]);
    }
    ...
    async complete(sessionId, data) {
        ...
        await run(`UPDATE practice_sessions SET ${sets.join(", ")} WHERE id=?`, params);
