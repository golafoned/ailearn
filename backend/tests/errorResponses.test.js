...
describe("Standardized error responses", () => {
    test("invalid refresh token returns standardized error", async () => {
        const res = await request(app)
            ...
            .send({ refreshToken: "badbadbadbad" });
    ...
    test("forbidden attempts listing when not owner", async () => {
        await request(app)
            ...
            .send({ email: "o@e.com", password: "Passw0rd!" });
        ...
        await request(app)
            .post("/api/v1/auth/register")
            .send({ email: "x@e.com", password: "Passw0rd!" });
        ...
        expect(list.body).toHaveProperty(
            "error.code",
    ...
    test("duplicate submit returns error code", async () => {
        await request(app)
            .post("/api/v1/auth/register")
            .send({ email: "d@e.com", password: "Passw0rd!" });
        const loginOwner = await request(app)
            .post("/api/v1/auth/login")
            .send({ email: "d@e.com", password: "Passw0rd!" });
        const tokenOwner = loginOwner.body.accessToken;
        const meOwner = await request(app)
            .get("/api/v1/auth/me")
            .set("Authorization", `Bearer ${tokenOwner}`);
        ...
        await request(app)
            .post("/api/v1/auth/register")
            .send({ email: "dup@e.com", password: "Passw0rd!" });
    ...
    test("expired test start returns gone code", async () => {
        await request(app)
            .post("/api/v1/auth/register")
            .send({ email: "e@e.com", password: "Passw0rd!" });
        const loginOwner = await request(app)
            .post("/api/v1/auth/login")
            .send({ email: "e@e.com", password: "Passw0rd!" });
        const tokenOwner = loginOwner.body.accessToken;
        ...
        await request(app)
            .post("/api/v1/auth/register")
            .send({ email: "ex@e.com", password: "Passw0rd!" });
        const loginU = await request(app)
            .post("/api/v1/auth/login")
            .send({ email: "ex@e.com", password: "Passw0rd!" });
        const tokenU = loginU.body.accessToken;
