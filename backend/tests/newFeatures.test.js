...
async function manualTest(
    ...
    const expires_at = new Date(
        Date.now() + (expired ? -60000 : 3600000)
    ).toISOString();
...
describe("New feature endpoints", () => {
    test("attempt detail hides correct answers for participant but shows correctness", async () => {
        ...
        const {
            ...
            code,
            questions,
        } = await manualTest(me.body.user.id, { questionCount: 2 });
        ...
        await request(app)
            ...
            .send({
                ...
                answers: [
                    { questionId: questions[0].id, answer: "A" },
                    { questionId: questions[1].id, answer: "C" },
                ],
            });
        const detail = await request(app)
            .get(`/api/v1/tests/attempt/${attemptId2}`)
            .set("Authorization", `Bearer ${participant.token}`);
    ...
    test("owner attempt detail shows correct answers", async () => {
        ...
        await request(app)
            ...
            .send({
                ...
                answers: [
                    { questionId: questions[0].id, answer: "A" },
                    { questionId: questions[1].id, answer: "D" },
                ],
            });
    ...
    test("leaderboard returns ordered scores", async () => {
        ...
        await request(app)
            .post("/api/v1/tests/submit")
            .send({
                attemptId: a2.body.attemptId,
                answers: [{ questionId: questions[0].id, answer: "A" }],
            }); // partial, one answered
        ...
        await request(app)
            .post("/api/v1/tests/submit")
            .send({
                attemptId: a3.body.attemptId,
                answers: questions.map((q) => ({
                    questionId: q.id,
                    answer: "B",
                })),
            }); // 0%
        const lb = await request(app).get(
            `/api/v1/tests/${testId}/leaderboard?limit=2`
        );
        expect(lb.status).toBe(200);
        expect(lb.body.leaderboard.length).toBe(2);
        expect(lb.body.leaderboard[0].score).toBeGreaterThanOrEqual(
            lb.body.leaderboard[1].score
        );
    }, 15000);
    ...
    test("close test prevents new starts", async () => {
        ...
        const me = await request(app)
            .get("/api/v1/auth/me")
            .set("Authorization", `Bearer ${owner.token}`);
        const { id: testId, code } = await manualTest(me.body.user.id, {
            questionCount: 1,
        });
