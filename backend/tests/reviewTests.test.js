...
async function generateBaseTestDirect(ownerId, { questionCount = 3 } = {}) {
    ...
    const test = await testRepo.create({
        ...
        source_text:
            "Cells are the basic unit of life. Mitochondria produce energy. Ribosomes synthesize proteins.",
        model: "dry-run",
...
describe("Review / Practice Generation", () => {
    ...
    test("review tests list and recommendations", async () => {
        ...
        await request(app)
            .post("/api/v1/tests/review")
            .set("Authorization", "Bearer " + token)
            .send({ strategy: "wrong_recent", attemptId, questionCount: 3 });
        const mine = await request(app)
            .get("/api/v1/tests/review/mine")
            .set("Authorization", "Bearer " + token);
        expect(mine.status).toBe(200);
        expect(Array.isArray(mine.body.items)).toBe(true);
        const rec = await request(app)
            .get("/api/v1/tests/review/recommendations")
            .set("Authorization", "Bearer " + token);
    ...
    test("review start forbidden for non-owner", async () => {
        ...
        const ownerId = JSON.parse(
            Buffer.from(tokenA.split(".")[1], "base64").toString()
        ).sub;
        const base = await generateBaseTestDirect(ownerId, {
            questionCount: 3,
        });
        const shell = await request(app).get("/api/v1/tests/code/" + base.code);
        const qIds = shell.body.questions.map((q) => q.id);
        ...
        const review = await request(app)
            ...
            .send({ strategy: "wrong_recent", attemptId, questionCount: 2 });
        const tokenB = await authFlow(app, "intruder@example.com");
        const forbidden = await request(app)
            .post("/api/v1/tests/start")
            .set("Authorization", "Bearer " + tokenB)
            .send({ code: review.body.code });
