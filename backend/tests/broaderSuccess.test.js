...
async function register(email, displayName) {
  ...
  return { token: login.body.accessToken, refresh: login.body.refreshToken, password };
}

describe("Broader success flows", () => {
  it("refresh rotation returns new tokens", async () => {
    const { token, refresh } = await register("rotate@example.com", "Rotator");
  ...
  it("logout revokes refresh tokens", async () => {
    const { token, refresh } = await register("logout@example.com", "Logger");
    ...
    const ref = await request(app).post("/api/v1/auth/refresh").send({ refreshToken: refresh });
